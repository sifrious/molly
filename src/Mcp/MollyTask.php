...
class MollyTask extends Tool
{
    public function handle(Request $request): Response|ResponseFactory
    {
        ...
        try {
            $result = match ($data['operation']) {
                ...
                'create' => ['task' => app(CreateTask::class)->handle($data['prompt'], $data['workspace'], $data['paths'] ?? [], $data['test_path'], nickname: $data['nickname'] ?? null)->toArray()],
                'from_plan' => ['task' => app(CreateTaskFromPlan::class)->handle($data['plan_id'], $data['prompt'], $data['workspace'], $data['paths'] ?? [], $data['test_path'], nickname: $data['nickname'] ?? null)->toArray()],
                'import_github' => ['task' => app(ImportGitHubIssue::class)->handle($data['issue_url'], $data['workspace'], $data['paths'] ?? [], $data['test_path'], nickname: $data['nickname'] ?? null)->toArray()],
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'paths' => $schema->array()->items($schema->string())->description('Optional additional relative files Molly may change. The required test is included automatically.'),
            'test_path' => $schema->string()->description('Required PHP test file under tests/. Selecting the test permits test edits.'),
