...
class PlanController
{
    ...
    {
        abort_unless(config('molly.typesafe.enabled') === true && is_string(config('molly.typesafe.api_key')) && trim(config('molly.typesafe.api_key')) !== '', 403);
    ...
    public function show(string $plan, ShowPlan $show, PlanningGuide $guide): View
    {
        ...
        return view('molly::plan', [
            ...
            'canSuggest' => config('molly.typesafe.enabled') === true && is_string(config('molly.typesafe.api_key')) && trim(config('molly.typesafe.api_key')) !== '',
            'steps' => $guide->steps(),
