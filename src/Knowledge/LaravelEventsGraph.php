...
final class LaravelEventsGraph
{
    ...
    public function build(string $version): array
    {
        ...
        if (! preg_match('/Laravel \d+ /', $document['title'])) {
            throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The bundled events guide needs a Laravel major version in its title.');
        }
        ...
        $documentSource = new GraphSource(
            'laravel', $version, 'documentation', 'events', $document['title'],
        ...
        foreach ($sections as $section) {
            $sectionNode = $this->node(
                $nodes, $version, 'documentation_section', 'docs:events#'.$section['slug'],
        ...
        $introduction = $nodes[$this->nodeId($version, 'documentation_section', 'docs:events#introduction')]
            ?? throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The events guide needs an Introduction section.');
        $events = $this->node($nodes, $version, 'concept', 'events', 'Events', [$documentSource->id()]);
        $this->edge($edges, $version, 'contains', $versionNode, $events, [$documentSource->id()]);
        $this->edge($edges, $version, 'documented_in', $events, $introduction, [$documentSource->id()]);

        $dispatcher = $this->symbol($sources, $nodes, $version, Dispatcher::class);
        $this->edge($edges, $version, 'uses', $events, $dispatcher['node'], [$dispatcher['source']]);
