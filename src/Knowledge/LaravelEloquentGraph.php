...
final class LaravelEloquentGraph
{
    ...
    public function build(string $version): array
    {
        ...
        if (! preg_match('/Laravel \d+ /', $document['title'])) {
            throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The bundled eloquent guide needs a Laravel major version in its title.');
        }
        ...
        $documentSource = new GraphSource(
            'laravel', $version, 'documentation', 'eloquent', $document['title'],
        ...
        foreach ($sections as $section) {
            $sectionNode = $this->node(
                $nodes, $version, 'documentation_section', 'docs:eloquent#'.$section['slug'],
        ...
        $introduction = $nodes[$this->nodeId($version, 'documentation_section', 'docs:eloquent#introduction')]
            ?? throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The eloquent guide needs an Introduction section.');
        $eloquent = $this->node($nodes, $version, 'concept', 'eloquent', 'Eloquent', [$documentSource->id()]);
        $this->edge($edges, $version, 'contains', $versionNode, $eloquent, [$documentSource->id()]);
        $this->edge($edges, $version, 'documented_in', $eloquent, $introduction, [$documentSource->id()]);

        $model = $this->symbol($sources, $nodes, $version, Model::class);
        $this->edge($edges, $version, 'uses', $eloquent, $model['node'], [$model['source']]);
