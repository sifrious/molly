...
final class LaravelTestingGraph
{
    ...
    public function build(string $version): array
    {
        ...
        if (! preg_match('/Laravel \\d+ /', $document['title'])) {
            throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The bundled testing guide needs a Laravel major version in its title.');
        }
        ...
        $documentSource = new GraphSource(
            'laravel', $version, 'documentation', 'testing', $document['title'],
        ...
        foreach ($sections as $section) {
            $sectionNode = $this->node(
                $nodes, $version, 'documentation_section', 'docs:testing#'.$section['slug'],
        ...
        $introduction = $nodes[$this->nodeId($version, 'documentation_section', 'docs:testing#introduction')]
            ?? throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The testing guide needs an Introduction section.');
        $pest = $this->node($nodes, $version, 'concept', 'pest', 'Pest', [$documentSource->id()]);
        $phpunit = $this->node($nodes, $version, 'concept', 'phpunit', 'PHPUnit', [$documentSource->id()]);
        $feature = $this->node($nodes, $version, 'configuration', 'tests/Feature', 'tests/Feature', [$documentSource->id()]);
        ...
        $this->edge($edges, $version, 'related_to', $pest, $phpunit, [$documentSource->id()]);
        $this->edge($edges, $version, 'tested_by', $feature, $pest, [$documentSource->id()]);

        $testCase = $this->symbol($sources, $nodes, $version, 'Illuminate\\Foundation\\Testing\\TestCase');
        $this->edge($edges, $version, 'uses', $phpunit, $testCase['node'], [$testCase['source']]);
        $this->edge($edges, $version, 'uses', $pest, $testCase['node'], [$testCase['source']]);
    ...
    private function symbol(array &$sources, array &$nodes, string $version, string $class): array
    {
        ...
        $node = $this->node(
            $nodes, $version, $reflection->isInterface() ? 'interface' : 'class', $class, class_basename($class), [$source->id()],
