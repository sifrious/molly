...
final class LaravelContainerGraph
{
    ...
    public function build(string $version): array
    {
        ...
        if (! preg_match('/Laravel \d+ /', $document['title'])) {
            throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The bundled container guide needs a Laravel major version in its title.');
        }
        ...
        $documentSource = new GraphSource(
            'laravel', $version, 'documentation', 'container', $document['title'],
        ...
        foreach ($sections as $section) {
            $sectionNode = $this->node(
                $nodes, $version, 'documentation_section', 'docs:container#'.$section['slug'],
        ...
        $resolution = $nodes[$this->nodeId($version, 'documentation_section', 'docs:container#zero-configuration-resolution')]
            ?? throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The container guide needs a Zero Configuration Resolution section.');
        $container = $this->node($nodes, $version, 'concept', 'container', 'Container', [$documentSource->id()]);
        $this->edge($edges, $version, 'contains', $versionNode, $container, [$documentSource->id()]);
        $this->edge($edges, $version, 'documented_in', $container, $resolution, [$documentSource->id()]);

        $symbol = $this->symbol($sources, $nodes, $version, Container::class);
        $this->edge($edges, $version, 'uses', $container, $symbol['node'], [$symbol['source']]);
