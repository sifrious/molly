...
final class LaravelValidationGraph
{
    ...
    public function build(string $version): array
    {
        ...
        if (! preg_match('/Laravel \d+ /', $document['title'])) {
            throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The bundled validation guide needs a Laravel major version in its title.');
        }
        ...
        $documentSource = new GraphSource(
            'laravel', $version, 'documentation', 'validation', $document['title'],
        ...
        foreach ($sections as $section) {
            $sectionNode = $this->node(
                $nodes, $version, 'documentation_section', 'docs:validation#'.$section['slug'],
        ...
        $introduction = $nodes[$this->nodeId($version, 'documentation_section', 'docs:validation#introduction')]
            ?? throw new RuntimeException('KNOWLEDGE_DOC_INVALID: The validation guide needs an Introduction section.');
        $validation = $this->node($nodes, $version, 'concept', 'validation', 'Validation', [$documentSource->id()]);
        $this->edge($edges, $version, 'contains', $versionNode, $validation, [$documentSource->id()]);
        $this->edge($edges, $version, 'documented_in', $validation, $introduction, [$documentSource->id()]);

        $validator = $this->symbol($sources, $nodes, $version, Validator::class);
        $this->edge($edges, $version, 'uses', $validation, $validator['node'], [$validator['source']]);
