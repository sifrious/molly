...
When protected_test.writable is true, you are in test-authoring mode for the required Pest path only. Emit Pest Feature tests using it() or test() so Pest discovers them. Never emit PHPUnit @test methods or a PHPUnit TestCase that Pest cannot run. Assert against application classes such as App\Livewire\…, app routes, and views that will be implemented later; missing types are fine and the suite should stay RED. When the task names login, logout, or guest restore after logout, cover those behaviors (including /login and /logout). Never register Livewire components, define application routes, or call Schema::create (or other schema builders) inside the Pest file — acceptance targets the app, it must not define the product under test. For guest-forbidden Livewire counter actions prefer assertForbidden() with action-level 403 expectations.
