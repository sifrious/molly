...
final class MollyKnowledgeIndexCommand extends Command
{
    ...
    public function handle(IndexLaravelKnowledge $laravel, IndexNativePhpKnowledge $nativephp, IndexTarpitKnowledge $tarpit): int
    {
        try {
            $namespace = (string) $this->argument('namespace');
            $result = match ($namespace) {
                'laravel' => $laravel->handle($this->option('laravel-version')),
                'nativephp' => $this->indexWithoutLaravelVersion($nativephp->handle(...), 'NativePHP'),
                'tarpit' => $this->indexWithoutLaravelVersion($tarpit->handle(...), 'Tarpit'),
            ...
            } elseif ($namespace === 'tarpit') {
                note('Tarpit notes '.$result['version'].' indexed. They are not a quality score and cannot override Pest.');
                table(['Sources', 'Nodes', 'Edges'], [[$result['sources'], $result['nodes'], $result['edges']]]);
                note('Database: '.$result['database']);
            } else {
    ...
    /**
     * @param  callable(): array<string, mixed>  $index
     * @return array<string, mixed>
     */
    private function indexWithoutLaravelVersion(callable $index, string $label): array
    {
        if (filled($this->option('laravel-version'))) {
            throw new RuntimeException('KNOWLEDGE_VERSION_INVALID: '.$label.' indexing does not use --laravel-version.');
        }
