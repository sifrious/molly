...
{
    protected $signature = 'molly:import {issue : GitHub issue URL} {--workspace= : Repository path} {--file=* : Repository-relative file Molly may change} {--test= : Pest test file Molly may change and must pass} {--name= : Task nickname} {--json : Print JSON only}';
    ...
    {
        try {
            $task = $action->handle((string) $this->argument('issue'), (string) ($this->option('workspace') ?: base_path()), $this->option('file'), trim((string) $this->option('test')), ...($this->option('name') === null ? [] : ['nickname' => (string) $this->option('name')]));
            if ($this->option('json')) {
