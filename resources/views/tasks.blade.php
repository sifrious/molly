...
@foreach($tasks as $task)<tr><th scope="row"><a href="{{ route('molly.tasks.show', $task->id) }}">{{ \Illuminate\Support\Str::limit($task->prompt, 100) }}</a></th><td>{{ $task->status }}</td><td>{{ $task->created_at }}</td></tr>@endforeach
</tbody></table></div>
@endif
@endsection
