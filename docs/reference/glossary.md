...
| Orb | Planned. A connected worker or execution endpoint with a verified stable identity. The actual connector and protocol remain to be selected. An Orb's presence and task associations are separate facts. | [Proposed design](../execution-targets.md), no implementation yet. |
| Orb task association | Planned. A confirmed link between an Orb and a task UUID, with connection, run or branch references and timestamps when available. Task renames preserve the association. | [Exact task lookup](../execution-targets.md#find-the-orb-associated-with-a-task), no implementation yet. |
