...
| `molly_task` | Creates and manages saved tasks, names, thread links, advice, GitHub comments, human approval, recorded pull requests, merge SHAs, handoff envelopes, and lifecycle requests. Show returns display status and recorded GitHub URLs. Approve, pr_opened, and merged require `approve=true` and do not open or merge a pull request. |
