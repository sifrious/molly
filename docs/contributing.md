...
The package tests use Pest and Orchestra Testbench. Laravel AI fakes test model responses without requiring Ollama. A live demo requires a host application and either an authenticated Amp account or an installed local Ollama model. Molly supplies the complexity commands.

`phpunit.xml` allows 512 MiB for the combined test suite. The limit applies to package tests, not the host application's runtime. Amp observation tests also exercise real subprocess timeouts and output limits with local fixtures. TypeSafe tests use HTTP fixtures, including malformed responses, low confidence, and missing credentials.

CI checks both Git and Composer download archives against a 256 KiB limit. Downloads include the cited planning guide and required licenses. Generated documentation, local Git metadata, and tests stay out of the package archive.
...
The Amp execution path passed a live bounded coding check. Amp proposed a PHP file and Pest test, Molly applied the selected files, Pest passed one test with one assertion, and parallel Amp Tarpit review passed. Amp's MCP doctor connected to Molly's original guide, plan, and task tools. The later connection tool has transport parity tests.

A real connection read with Amp `0.0.1788782430-g6feb0e` returned an observed, disconnected executor with `working=false` and raw executor type `sandbox`. The result did not establish an Orb identity. No live TypeSafe result has been verified because this environment has no configured TypeSafe key.
