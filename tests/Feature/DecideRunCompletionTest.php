...
it('does not complete when Pest reports a pass without the required test file', function () {
    config(['molly.verification' => ['pest' => 'required', 'tarpit' => 'required', 'parallel_join' => 'required']]);

    $decision = app(DecideRunCompletion::class)->handle([
        'mode' => 'serial',
        'verification' => ['status' => 'passed', 'tests' => 1, 'assertions' => 1, 'identified_required_test' => false],
        'review' => completeReview(),
    ], ['app/Greeting.php' => '<?php return "Hello";']);
